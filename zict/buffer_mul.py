...
class BufferMul(ZictBase):
    ...
    LRU
    """
    def __init__(self, fast, fast_2, slow, n, policy, weight=lambda k, v: 1,
                 fast_to_slow_callbacks=None, slow_to_fast_callbacks=None):
        
        self.fast = LRU(n*0.5, fast, weight=weight, on_evict=[self.fast_to_slow])
        self.fast_data = LFU(n*0.5, fast_2, weight=weight, on_evict=[self.fast_to_slow])
    ...
    def slow_to_fast(self, key):
        ...
        if self.weight(key, value) <= self.n:
            del self.slow[key]
            if 'from-delayed' not in key:
                self.fast[key] = value
            else:
                self.fast_data[key] = value
    ...
    def __getitem__(self, key):
        ...
        elif key in self.fast_data:
            return self.fast_data[key]
    ...
    def __delitem__(self, key):
        if key in self.fast:
            ...
            del self.fast_data[key]
        else:
            raise KeyError(key)
        
    def update_reference_count(self,key,value):
        if policy == 'lrc':
            if 'from-delayed' in key:
                self.fast_data.update_ref_count(key,value)
