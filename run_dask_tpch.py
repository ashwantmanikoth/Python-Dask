...
def filter_primary_partition_for_both_sorted_relation(foreign_df, foreign_col_list, primary_col_list, primary_df=None):
    ...
    primary_part = primary_df[
        (primary_df[primary_col] >= key_value_first) & (primary_df[primary_col] <= key_value_last)]
    results = partition_join(foreign_df, foreign_col_list, primary_part, primary_col_list)
    return results


def filter_primary_partition_for_one_sorted_relation(foreign_df, foreign_col_list, primary_col_list, heaviside,
                                                     primary_df=None, sparse_index=None):
    import numpy as np
    primary_col = primary_col_list[0]
...
dli.set_map_partition_func(filter_primary_partition_for_both_sorted_relation,
                           filter_primary_partition_for_one_sorted_relation, partition_join)
...
sql_test = """SELECT l_suppkey, SUM(l_extendedprice * (1 - l_discount)) AS total_revenue, 1 AS batch_num
  FROM lineitem
  WHERE l_shipdate >= '1996-01-01' AND l_shipdate < DATE_ADD('1996-01-01', INTERVAL '90' DAY)
  GROUP BY l_suppkey
  LIMIT 100000;"""
...
output = 'query_number : ' + str(query_number) + ' scale_factor : ' + scale_factor + ' number_or_runs : ' + str(
    number_or_runs) + '\n'
result = ""
total_time = 0
