...
class BloomFilter(object):
    ...
    def __init__(self, items_count, fp_prob, num_filters=1):
        '''
        items_count : int
        ...
        # Total number of multidimensional bloom filters
        self.num_filters = num_filters

        # Size of bit array for single dimension to use
        ...
        self.bit_array = bitarray(self.size * self.num_filters)
    ...
    def add(self, df):
        ...
        for k in range(self.num_filters):
            ...
            for i in range(self.hash_count):
                ...
                #digest = self.bloom_hash(item, i+1) % self.size + k * self.size
                [digests.append(self.bloom_hash(item, i+1) % self.size + k * self.size) for item in values]
    
                # set the bit True in bit_array
                #[self.bit_array.insert(digest, True) for digest in digests]
                for digest in digests:
                    self.bit_array[digest] = True
                
                [print(self.bit_array[digest], digest) for digest in digests]
    ...
    def simple_hash(self, val, prime1, prime2, prime3):
        """Compute a hash value from a list of integers and 3 primes"""
        result = ((val + prime1) * prime2) % prime3
        return result
    
    
    def hash1(self, val):
        """Basic hash function #1"""
        return self.simple_hash(val, self.MERSENNES1[0], self.MERSENNES1[1], self.MERSENNES1[2])
    
    
    def hash2(self, val):
        """Basic hash function #2"""
        return self.simple_hash(val, self.MERSENNES2[0], self.MERSENNES2[1], self.MERSENNES2[2])
    
    def bloom_hash(self, val, seed):
...
bloomf = BloomFilter(n,p,4)
print("Size of bit array:{}".format(bloomf.size))
print("False positive Probability:{}".format(bloomf.fp_prob))
print("Number of hash functions:{}".format(bloomf.hash_count))

bloomf.add(df_n)
status = bloomf.check([11,20,11,12])
print(status)
