...
clientHadoop = Client('131.202.243.10:8786')
...
DATA_FILE_PATH = 'hdfs:///input/datasets_for_dask_DB/data_10/'
...
#            customer=customer.set_index(self.column_mappings["customer"][0], divisions=divisions)
            ...
            indexed_col2 =self.column_mappings["orders"][1]
            merged_table_orders=customer.merge(orders,left_on=self.column_mappings["customer"][0],right_index=True)
#            merged_table_orders=customer.merge(orders,left_on=self.column_mappings["customer"][0],right_on=self.column_mappings["orders"][1])
#            merged_table_orders=customer.merge(orders, left_index=True, right_index=True)
            #merged_table_orders = clientHadoop.persist(merged_table_orders)
#             print "Cache Data:"
#             print cache.cache.data
#             print "Data Score:"
#             print cache.cache.heap.heap
...
#             
            ...
            #merged_table_nation2 = merged_table_nation
            #start1 = timer()
            #merged_table_nation2 = merged_table_nation2.nlargest(20,columns=["revenue"]).compute()
            #end1 = timer()
            #print merged_table_nation2
            #print "Time taken for dataframe.compute(): ", str(end1 - start1)
            
            
            #start = timer()
            #merged_table_nation=clientHadoop.persist(merged_table_nation)
            #final=clientHadoop.compute(merged_table_nation.nlargest(20,columns=["revenue"]))
