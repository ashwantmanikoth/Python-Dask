...
class ControlFlowGraph(object):
    ...
    def compute_liveness(self):
        ...
        live_in = {i: copy.copy(self.graph.nodes[i]['uses'])
                   for i in self.graph}
        ...
        while True:
            ...
            for i in self.graph:
                ...
                def_var = self.graph.nodes[i]['def_var']
                def_set = set()
    ...
    def __inline_node(self, dest_node, target_node):
        ...
        target_op = self.graph.nodes[target_node]['op']
        assert isinstance(target_op, StoreTemp)
        target_inner_op = target_op.input

        dest_op = self.graph.nodes[dest_node]['op']

        var = self.graph.nodes[target_node]['def_var']
        assert var is not None

        new_op = inline_operator(dest_op, var, target_inner_op)
        self.graph.nodes[dest_node]['op'] = new_op

        # The merged node uses the union of the previous nodes input variables
        uses_set = self.graph.nodes[dest_node]['uses']
        uses_set.remove(var)
        uses_set.update(self.graph.nodes[target_node]['uses'])
    ...
    def apply_chaining(self):
        ...
        while _continue:
            ...
            for nodeB, nodeA in sliding_window(reversed(self.sorted_vertices)):
                ...
                if isinstance(self.graph.nodes[nodeB]['op'], UntilConvergence):
                    continue  # start of do/until convergence

                def_var = self.graph.nodes[nodeA]['def_var']
                if not def_var:
                    continue

                uses = self.graph.nodes[nodeB]['uses']
    ...
    def dead_code_elimination(self):
        ...
        while _continue:
            ...
            for node in self.graph:
                ...
                def_var = self.graph.nodes[node]['def_var']
