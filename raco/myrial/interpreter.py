...
class ExpressionProcessor(object):
    ...
    def select(self, args):
        ...
        op = self.bagcomp(args.from_, args.where,
                          args.select, args.groupby ,args.orderby, args.limit)
        if args.distinct:
    ...
    def bagcomp(self, from_clause, where_clause, emit_clause, groupby_clause,
        ...
        ##awatson
        ## remove the indexes keep original column names so easier in future
        emit_args = [(name, multiway.rewrite_refs(sexpr, from_args, info))  
                     for (name, sexpr) in emit_args]

        ## awatson
        ##Added the groupby clause 
        ##remove the indexes keep original column names so easier in future
        if groupby_clause:
            temp_names = []
            for (name, sexpr) in emit_args:
                ...
                if name not in groupby_clause and not raco.expression.expression_contains_aggregate(sexpr):
                    print(name,sexpr)
                    raise InvalidStatementException(
                        "Statement emit column not in the groupby")
                    
            for i in groupby_clause:
                if i not in temp_names:
                    emit_args.append((i,raco.expression.NamedAttributeRef(i)))
        statemods = multiway.rewrite_statemods(statemods, from_args, info)

        #awatson
        # it seems to check for implicit groupby calls whatever this means 
        ...
        ### awatson these are two clauses we want to upgrade removed this unneeded constraints
        if orderby_clause:
#             if limit_clause is None:
#                 raise InvalidStatementException(
#                     "An ORDER BY clause must be accompanied by a LIMIT clause")
            orderby_cols, orderby_ords = zip(*orderby_clause)
            orderby_idxs = [get_unnamed_ref(col, op.scheme()).position # awatson place to change column index to names
                            for col in orderby_cols]
            op = raco.algebra.OrderBy(input=op,
                                      sort_columns=orderby_cols, #awatson to return column names opposed to 
                                      ascending=orderby_ords)

        if limit_clause: # romved this unneeded constraints
...
#                     "A LIMIT clause must be accompanied by an ORDER BY clause")
    ...
    def join(self, left_target, right_target):
        ...
        join_conditions = [raco.expression.EQ(x, y) for x, y in
                           zip(left_target.columns, right_target.columns)]
