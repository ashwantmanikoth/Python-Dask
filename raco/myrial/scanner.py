...
comprehension_keywords = ['SELECT', 'AS', 'EMIT', 'FROM', 'WHERE', 'ORDERBY' ,'GROUPBY'] # awatson
...
builtins = ['EMPTY', 'WORKER_ID', 'SCAN', 'COUNTALL', 'COUNT', 'STORE',
            ...
            ,'DATE','INTERVAL', 'MONTH', 'DAY', 'YEAR'] # awatson custom ones here 
...
def t_BLOB_LITERAL(t):
    ...
    #t.value = bytes(t.value[2:-1].decode("string_escape"))    #skdas python3
    t.value = bytes(bytes(t.value[2:-1], "utf-8").decode("unicode_escape"))
    return t
...
def t_STRING_LITERAL(t):
    r'"([^\\\n"]|\\.)*"|\'([^\\\n\']|\\.)*\''
    #t.value = t.value[1:-1].decode("string_escape")  #skdas python3
    t.value = bytes(t.value[1:-1], "utf-8").decode("unicode_escape")
    return t

# def t_DATETIME_LITERAL(t):
#     #r'([1-2][0-9][0-9][0-9])(.|-|)(0[1-9]|1[0-2])(.|-|)(0[1-9]|1[0-9]| 2[0-9]|3[0-1])'
#     r'DATE\s"([^\\\n"]|\\.)*"|\'([^\\\n\']|\\.)*\''
